...
from utils import has_all_types, is_valid_email
    ...
    with open(userInfoPath if role == "user" else driverInfoPath) as f:
        ...
        if (not has_all_types(username)):
            return {
                ...
                False,
                "Message":
                "username must have alphabets, numbers and special character"
            }
    return {"Status": True}
...
def isPhoneNumberValid(phone_number):
    if (phone_number.isnumeric() and len(phone_number) == 11):
        ...
        return {
            ...
            False,
            "Message":
            "Enter Valid Phone number, Should be number and has 11 digits"
        }
...
def isPasswordValid(password):
    ...
    if (not has_all_types(password)):
        return {
            ...
            False,
            "Message":
            "password must have alphabets, numbers and special character"
        }
    return {"Status": True}


def isConfrimPasswordValid(password, confrim_password):
...
def isCarModelValid(car_model):
    if (car_model.isnumeric() and len(car_model) == 4):
        return {"Status": True}
    else:
        return {"Status": False, "Message": "Enter Valid Car Model"}


def isCarNumberValid(car_number):
    if (car_number.isalnum() and len(car_number) > 4):
        return {"Status": True}
    else:
        return {"Status": False, "Message": "Enter Valid Car Number"}
